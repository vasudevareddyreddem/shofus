...
class Subcategory extends Admin_Controller {
	...
	public function index()
	{
		//$this->load->view('welcome_message');
		 $config = [
	   'base_url'   => base_url('seller_admin/subcategory/index'),
	   'per_page'   => 20,
       ...
       $data['subcatdata'] = $this->subcategory_model->limit($config['per_page'], $this->uri->segment(4) )->getsubcat();
		...
		$this->template->write_view('content', 'seller_admin/subcategory/index', $data);
	...
	public function create()
	{
       ...
       //$data['itemdata'] = $this->category_model->getitems();
		...
		$this->template->write_view('content', 'seller_admin/subcategory/add');
		$this->template->render();
	...
	"seller_id" => $this->session->userdata('seller_id'),
           ...
           $res=$this->subcategory_model->insert($data);
	        ...
	        // return redirect('admin/accomodation');
	         ...
	         echo "<script>window.location='".base_url()."seller_admin/subcategory';</script>";
...
public function edit()
{
	...
	$this->template->write_view('content', 'seller_admin/subcategory/edit', $data);
		$this->template->render();
