...

{
	...
	public function uploadproducts(){
		...
		$seller_location=$this->products_model->get_store_location($this->session->userdata('seller_id'));	
		/* for other poruducts*/
			if($cat_id[0]==1 && $cat_id[0]!=2 ){
				if(substr($_FILES['categoryes']['name'], 0, 13)=='otherproducts'){
				...
				if (($fileExt == "xlsx") && ($fileSize < $limitSize)) {
					...
					if(count($data['errors'])<=0){
						...
						//echo '<pre>';print_r($data);
								...
								if(isset($data[27])&& $data[27]!=''){
									...
									copy($data[27], $path.$imagename11);
									
								}
								
								
								
								//$sizes=explode(",",$data[6]);
								//$productspecificationlist= array_combine($dada[14],$data[15]);
							...
							//echo '<pre>';print_r($adddetails);exit;
								...
								if(count($results)>0){
									/* for size purpose*/
									foreach(explode(",",$data[6]) as $listsizes){
									if($listsizes !=''){
										$addsizesdata=array(
										'item_id' =>$results,
									...
									/* for size purpose*/
									/* for color purpose*/
									foreach(explode(",",$data[7]) as $listcolors){
										if($listcolors !=''){
												$addcolorsdata=array(
												'item_id' =>$results,
					...
					$this->session->set_flashdata('error','Please upload Correct file');
					redirect('/seller/products/create');
				}
				
			}
			/*---*/
			/* for fashion poruducts*/
			if($cat_id[0]==2){
					if(substr($_FILES['categoryes']['name'], 0, 15)=='fashionproducts'){					
						...
						foreach($arry as $key=>$fields)
							{
								if(isset($fields[1]) && $fields[1]!='' && $fields[2]!='' && $fields[3]!='' && $fields[4]!='' && $fields[5]!=''  && $fields[6]!='' && $fields[7]!=''  && $fields[8]!='' ){
									...
									if(empty($fields[16])) {
										$data['errors'][]="Sleeve / Fitting type is required. Row Id is :  ".$key;
									$error=1;
									}else if($fields[16]!=''){
										$regex ="/^[ A-Za-z0-9_@.}{@#&`~\\/,|=^?$%*)(_+-]*$/"; 
										if(!preg_match( $regex, $fields[16]))	  	
										{
										$data['errors'][]='Sleeve / Fitting type wont allow "  <> []. Row Id is :  '.$key.'<br>';
										$error=1;
										}
									}
									if(empty($fields[17])) {
									...
									if(empty($fields[18])) {
										$data['errors'][]="Season is required. Row Id is :  ".$key;
									$error=1;
									}else if($fields[18]!=''){
										$regex ="/^[ A-Za-z0-9_@.}{@#&`~\\/,|=^?$%*)(_+-]*$/"; 
										if(!preg_match( $regex, $fields[18]))	  	
										{
										$data['errors'][]='Season wont allow "  <> []. Row Id is :  '.$key.'<br>';
										$error=1;
										}
									}
									if(empty($fields[19])) {
									...
									if(empty($fields[20])) {
										$data['errors'][]="Gender is required. Row Id is :  ".$key.'<br>';
										$error=1;
									}else if($fields[20]!=''){
										$regex ="/^[0-2]+$/"; 
										if(!preg_match( $regex, $fields[20]))
										{
										$data['errors'][]="Gender must be digits. Row Id is :  ".$key.'<br>';
										$error=1;
										}
									}
									if(empty($fields[21])) {
