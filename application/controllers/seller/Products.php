...
class Products extends Admin_Controller {
	...
	{	    
		$items = trim($this->input->get('term', TRUE)); 
		$this->db->select('item_name');
    	$this->db->like('item_name', $items);
 ...
 //  	}
	...
	// {
	// 	$this->db->select('item_name');
 //     	$this->db->like('item_name');
 //     	$query = $this->db->get('items');
 //      	if($query->num_rows() > 0){
 //        foreach ($query->result_array() as $row){
 //          $row_set[] = $row['item_name'];
 //        }
 //        echo json_encode($row_set);exit;;
	// 	}
	// }
		...
		public function search_items()
{
	...
	public function control_areas(){
    $term = $this->input->get('term', TRUE);
    $countries = $this->products_model->get_areas($term);
