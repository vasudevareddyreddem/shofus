...

{
	...
	public function get_product_detail_review_count($id){
		$date = new DateTime("now");
 		$curr_date = $date->format('Y-m-d h:i:s A');
		$this->db->select('products.*,top_offers.*')->from('products');
		$this->db->join('top_offers', 'top_offers.item_id = products.item_id', 'left');
		...
		$return['details']['average']=$this->product_reviews_avg($query['item_id']);
		$return['details']['catergories']=$this->product_review_count($query['item_id']);
		if(!empty($return))
			{
			return $return;
	...
	public function product_reviews_avg($item_id){
		 
		$sql = "SELECT AVG(rating) as ratingavg FROM ratings WHERE item_id ='".$item_id."'";
		return $this->db->query($sql)->row_array();	
		 
	 }
	 public function product_review_count($item_id){
		 
		$sql = "SELECT COUNT(item_id) as reviewcount FROM ratings WHERE item_id ='".$item_id."'";
		return $this->db->query($sql)->row_array();	
